...
from tr1604_sim_v62 import App as V62App, MENUS, G, BR, D, Y, C, SM, FN, TI
from tr1604_sim_v6 import GRID

FW = "5.0.1"
BUILD = "desktop-v7-002"
BG = "#020b05"
MENU_WIDTH = 300
...
class App(V62App):
    ...
    def draw_measure(self, w, h):
        ...
        if marker.enabled:
            if antenna:
                mtext = f"M{self.s.selected+1}  {marker.f:.6f} MHz   SWR {self.swr(marker.f):.2f}   RL {self.return_loss(marker.f):.2f} dB"
            else:
        ...
        self.txt(px0, status_y, f"TRACE A LIVE   MEM B {'ON' if self.mem_on else 'OFF'}   {footer_delta}   AUTO {'ON' if self.s.auto_track else 'OFF'}   {tg}", Y if self.s.delta else G, SM)
